...
<body>
	...
	<section class="main-content">
		...
		<div class="formFields clearfix">
			<c:url var="searchAction" value="/userManagement/searchUser"></c:url>
			<c:url var="checkUserExistsEdit" value="/checkUserExistsEdit" />
			<%-- <c:url var="checkUserNameExistsEdit" value="/checkUserNameExistsEdit" /> --%>
			
			<c:url var="editAction" value="/users/editUser"></c:url>
			...
			<form:form action="${editAction}" commandName="users" method="POST"
					...
					<div class="col50per">
						...
						<div class="inputField">
							<form:input path="userName" id="txtuserName"
								...
								<form:hidden path="userNamehidden" id="txtuserNameHidden"/>
						</div>
					</div>
					<div class="col50per">
						...
						<div class="inputField">
							<form:password path="userPassword" id="password"
								class="input-control" data-validation="required" readonly="true" />
						</div>
					</div>
				</div>
				...
				<div class="formRow">
					<div class="col50per">
						<div class="fieldLbl">
							<label>Role Id <span class="req">*</span>:
							</label>
						</div>
						<div class="inputField">
							<form:select path="role_id" id="drproleId" class="select-control"
								data-validation="required">
								<option value="" selected="true">-Select-</option>
								<option value="1">Admin</option>
								<option value="2">BundleEM / EM</option> 
								<option value="3">Resource Manager</option> 
								<option value="4">Read Only User</option>
								<option value="5">BIS PMO</option>
								<option value="6">RM PMO</option>
							</form:select>
						</div>
					</div>
				</div>
				...
				<div class="text-center">
					<input type="submit" value="Save" title="On-board" id="btnSubmit"
						class="btn-primary" />
					<!--  <input type="Reset" value="Reset" title="Reset" id="btnReset" class="btn-primary" /> -->
					  <a href="${pageContext.request.contextPath}/userSearch" title="Back" class="btn-primary" style="display:inline-block;">Back</a> 
				</div>
		...
		$(document)
				.ready(
						function() {
							...
							$("#txtuserName").val('${users.userName}');
							$("#txtuserNameHidden").val('${users.userName}');
							$("#password").val('${users.userPassword}');
							$("#drproleId").val('${users.role_id}');
												...
												$
														.getJSON(
																'${checkUserExistsEdit}',
																{
																	...
																	userNamehidden : $('#txtuserNameHidden').val(),
															        ajax : 'true'
																},
																function(data) {
																	if (data) {
																			...
																			alert("User Name Id already exists . Please try another one . ");
																			$('#txtuserName').val("");
							...
							$("#btnSubmit")
									.click(
											function() {
												...
												/* if (passval == 0) {
													alert("Please enter the Password . Default password is 'pass'");
													return false;
												} */
