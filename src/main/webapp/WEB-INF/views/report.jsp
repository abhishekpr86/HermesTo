...
<body>
	...
	<section class="main-content">
		...
		<div class="formFields clearfix">
			<c:url var="searchAction" value="/report/search"></c:url>
			...
			<form:form action="${searchAction}" modelAttribute="employee"
				...
				<div class="formRow">
					...
					<div class="col50per">
						...
						<form:radiobutton path="empType" value="All" id="rdALlEmpCheck"
							name="rdoGroup" class="input-control1" checked = "false" />
						<label>All</label>
					</div>
				</div>
				<div class="formRow">
					...
					<div class="col50per">
						<div class="fieldLbl">
							<label>Actual Off-boarding Date (dd/mm/yyyy) : </label>
						</div>
						<div class="inputField">
					...
					<div class="formRow" style="margin-top:1px">
					<div class="col50per">
						<div class="fieldLbl">
							<label>BIS : </label>
						</div>
					...
					<div class="col50per">
						...
						<form:radiobutton path="resourceStatus" value="ActiveResources" id="rdActiveEmp" name="rdoGroup" class="input-control" checked = "false" />
							<label>Active Resources</label>
					
						<form:radiobutton path="resourceStatus" value="BillableResources" id="rdBillEmp" name="rdoGroup" class="input-control"/>
							<label>Billable Resources</label>
					
						<form:radiobutton path="resourceStatus" value="ShadowResources" id="rdshadEmp" name="rdoGroup" class="input-control"/>
							<label>Shadow Resources</label>
					
						<form:radiobutton path="resourceStatus" value="AllResources" id="rdAllEmp" name="rdoGroup" class="input-control"  />
				...
				<div class="text-center">
					...
					<button type="button" id="downloadResReport" class="btn btn-primary">Download</button>
				</div>
			</form:form>
		</div>
			...
			<table id="summaryTbl" class="summary-view-table dt-responsive">
				<thead>
					<tr>		
						...
						<th width="80">GG ID</th>
						<th width="80">Corp ID</th>
						<th width="80">PSA ID</th>
						<th width="80">VM Number</th>
						...
						<th width="50">Manager Email</th>
						<!-- <th width="50">HRA Request</th> -->
						<th width="50">Staff JIRA</th>
						...
						<th width="50">Replacement Corp ID</th>
	     	        	...
	     	        	<th width="50">PSA ID Req.</th>
	     	        	<th width="50">PSA ID Gen.</th>
	     	        	<!-- <th width="50">Upper V Cycle</th> -->
	     	        	<th width="50">Time & Material</th>
	     	        	<th width="50">Offboard Comment</th>
	     	        	<th width="50">EM Submit Date</th>
	     	        	<th width="50">RM Submit Date</th>
	     	        	<th width="50">BIS PMO Submit Date</th>
	     	        	<th width="50">RM PMO Submit Date</th>
	     	        	<!-- <th width="50">Core Team</th> -->
	     	        	<th width="50">Onboarding Buddy</th>
	     	        	<th width="50">Onboarding Email</th>
				...
				<tbody>
					<c:forEach items="${employeeSearchList}" var="employee">
						...
						<tr>	
							...
							<c:choose>
	    						<c:when test="${employee.ggId ne null}">
									<td>${employee.ggId}</td>
								</c:when> 
								<c:otherwise>
									<td>-</td>
						    ...
						    <td>${employee.demandType.type}</td>
					    	...
					    	<td>${employee.requestedDate}</td>
							<td>${employee.generatedDate}</td>
							<%-- <td>${employee.upperVCycle}</td> --%>
							<%-- <td>${employee.timeNMaterial}</td> --%>
							<td>${employee.timeMat}</td>
							<td>${employee.offboardcomment}</td>
							
							<c:choose>
	    						<c:when test="${employee.preonbemp.EMSubmitDT ne null}">
									<td>${employee.preonbemp.EMSubmitDT}</td>
								</c:when> 
								<c:otherwise>
									<td>-</td>
								</c:otherwise>
							...
							<c:choose>
	    						<c:when test="${employee.preonbemp.RMSubmitDT ne null}">
									<td>${employee.preonbemp.RMSubmitDT}</td>
								</c:when> 
								<c:otherwise>
									<td>-</td>
								</c:otherwise>
							...
							<c:choose>
	    						<c:when test="${employee.preonbemp.BISPMOSubmitDT ne null}">
									<td>${employee.preonbemp.BISPMOSubmitDT}</td>
								</c:when> 
								<c:otherwise>
									<td>-</td>
								</c:otherwise>
							...
							<c:choose>
	    						<c:when test="${employee.preonbemp.RMPMOSubmitDT ne null}">
									<td>${employee.preonbemp.RMPMOSubmitDT}</td>
								</c:when> 
								<c:otherwise>
									<td>-</td>
								</c:otherwise>
							</c:choose>
							<%-- <td>${employee.coreTeam}</td> --%>
						<c:if test="${employee.buddy ne null}">
							 <td>${employee.buddy.firstName}&nbsp;${employee.buddy.lastName}&nbsp;(${employee.buddy.corpId})</td>
						 </c:if> 
							<td>${employee.onboardingEmail}</td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
		...
		$(document)
				.ready(
						...
						/* 	$("#btnExport").click(function() {
							...
							var btn = document.getElementById("downloadResReport");		
							
					$("#downloadResReport").click(function(){	
						
						$("#downloadResReport").attr("disabled", true);
						//$("#downloadResReport").css("display", "none");
						btn.innerHTML = 'Downloading';
						
						var empType="";		
						if($('#rdIntEmpCheck').is(':checked')){
							empType="Internal";
						}else if($('#rdExtEmpCheck').is(':checked')){
							empType="External";
						}else{
							empType="All";
						}
						...
						var resStatus ="";
						if($('#rdActiveEmp').is(':checked')){
							resStatus ="ActiveResources";
						}else if($('#rdBillEmp').is(':checked')){
							resStatus ="BillableResources";
						}else if($('#rdshadEmp').is(':checked')){
							resStatus = "ShadowResources";
						}else{
							resStatus = "AllResources";
						}
						...
						if($("#drpbis").val() != ""){
							bisid =$("#drpbis").val();
								...
								$.ajax({
							 		...
							 		url : "/onboarding/ResReportExcelExport",
							 		dataType : 'text',
							 		data: {  country : countryid, actualoffdate : $("#txtActEndDate").val(), joinidate : $("#txtJoiningDate").val(), resStatus : resStatus, empType : empType, bis : bisid},
							 		success : function(response) {	
							 			
							 			console.log(response);
						  	 		 window.location.href = '/onboarding/downloadReportFile/' + response; 
						  	 		 
						  	 		$("#downloadResReport").attr("disabled", false);
								 	//$("#downloadResReport").css("display", "");
